...
def merge_data(eia_data,temp_region,yr):
    ...
    if pandas.to_datetime(str(yr)).is_leap_year:
        ...
        feb29 = feb29.merge(nrel_data,left_index=True,right_index=True\
                            ).drop(\'timestamp_y',axis=1).set_axis(['timestamp','ghi', 'dni','ws'\
...
def insert_into_db(eia_data,remp_region,yr):
    ...
    for i in range(len(temp_data)):
        cur.execute("INSERT INTO data (region, time_stamp, demand, net_generation, net_generation_solar, \
                    ghi, dni, windspeed, updated)\
